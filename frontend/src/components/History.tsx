            ...
            <div className="h-full overflow--y-auto overflow-x-hidden">
                {history.map((i) => (
                    <HistItem
                        ...
                        domain={edit.domain}
                        tooltip={tooltip}
                        sameUser={i.user === edit?.user?.username}
                        isCreate={edit?.type === 'create'}
                    />
                ))}
            </div>
        </div>
...
const HistItem = memo(function HistItem({
    ...
    return (
        <div
            onClick={handleClick}
            className={
                'text-[0.85rem] not-last:after:w-[90%] not-last:after:h-[0.5px] not-last:after:bottom-0 not-last:after:left-1/2 not-last:after:-translate-x-1/2 not-last:after:bg-neutral-700 not-last:after:absolute relative p-2 text-white [&:hover]:text-white hover:bg-neutral-800 transition' +
                ` ${isCurr ? 'bg-neutral-800!' : ''} ${isOld ? 'bg-[#202020]' : ''}`
            }
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center">
                    ...
                    <a
                        ...
                        className={`text-neutral-300 truncate ${sameUser ? `text-blue-400! ${obj.sameuser || isCreate ? 'font-bold' : ''}` : ''}`}
                        target="_blank"
